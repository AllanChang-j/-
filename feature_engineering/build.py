...
from feature_engineering.technical import build_common_ta_features, build_technical_features, candidate_feature_columns
...
def build_feature_frame(df: pd.DataFrame, config: dict[str, Any]) -> tuple[pd.DataFrame, list[str]]:
    ...
    profile = str(config.get("features", {}).get("profile", "full"))
    if profile == "common_ta":
        features = build_common_ta_features(df, lag_windows=lag_windows)
    else:
